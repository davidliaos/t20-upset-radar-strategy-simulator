...
def test_rank_notable_upsets_orders_by_abs_elo(synthetic_matches_df):
    ...
    n = len(df)
    df["elo_diff"] = [5.0, 60.0, 25.0, 80.0, 10.0, 40.0, 12.0, 18.0, 22.0][:n]
    df["is_upset"] = ([0, 1, 0, 1, 0, 1] * ((n // 6) + 1))[:n]
...
def test_matchup_volatility_profile_defaults_upset_rate_without_column(synthetic_matches_df):
    df = synthetic_matches_df.drop(columns=["is_upset"], errors="ignore").copy()
    profile = matchup_volatility_profile(df, "TeamA", "TeamB")
    assert profile["matches"] >= 1
    assert profile["upset_rate"] == 0.0


def test_build_missed_upsets_audit_returns_sorted_rows(synthetic_matches_df):
    df = synthetic_matches_df.copy()
    n = len(df)
    df["is_upset"] = ([1, 1, 0, 0, 1, 0] * ((n // 6) + 1))[:n]
    df["pred_is_upset"] = ([0, 1, 0, 0, 0, 0] * ((n // 6) + 1))[:n]
    df["favorite_team"] = df["team1"]
    df["team1_win_prob"] = ([0.9, 0.2, 0.6, 0.7, 0.8, 0.4] * ((n // 6) + 1))[:n]
...
def test_curated_narratives_and_summary(synthetic_matches_df):
    ...
    n = len(df)
    df["is_upset"] = ([0, 1, 0, 1, 0, 1] * ((n // 6) + 1))[:n]
    df["elo_diff"] = ([10, -30, 5, -40, 8, -20] * ((n // 6) + 1))[:n]
...
def test_notable_upset_sentence_handles_duplicate_team_names() -> None:
    out = notable_upset_sentence("TeamA", "TeamA", "World Cup", "2025-01-01")
    assert out == "TeamA at World Cup on 2025-01-01."
